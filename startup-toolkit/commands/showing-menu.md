...
3. **Gather environment details** — Ask targeted follow-up questions based on the use case: operating system, programming language/framework, Datadog site (default US1), whether they have an API key, container runtime, cloud provider, etc. Only ask what is needed for the selected task.
4. **Fetch live documentation** — Use the fetching-datadog-docs skill to look up the current documentation for the selected use case. Always start from `llms.txt` to find the right page, then fetch the full `.md` content. Do not skip this step.
5. **Execute the steps** — Walk the user through the documented steps. Run install commands, edit configuration files, and set environment variables as needed. Explain each step briefly.
6. **Verify** — After completing the setup, run a verification step (e.g., `datadog-agent status`, check the Datadog UI, confirm traces/logs/metrics are flowing). Confirm success with the user before wrapping up.
...
```
Datadog Getting Started — What would you like to do?
