...
description: >-
  ...
  It applies whenever the user asks "how do I configure X in Datadog", "what are the
  ...
  It also applies when the user mentions Datadog docs, llms.txt, Datadog documentation lookup,
  or Datadog integration references — even if they don't explicitly say "look up documentation".
  This skill also covers requests like "how do I troubleshoot the Datadog Agent" or questions
  about Agent connectivity, log collection, or metric submission issues.
version: "1.1.0"
author: datadog-labs
...
curl -s https://docs.datadoghq.com/llms.txt | grep -iE "apm|tracing"
...
| EU1 | https://docs.datadoghq.com | https://api.datadoghq.eu |
| AP1 | https://docs.datadoghq.com | https://api.ap1.datadoghq.com |
| AP2 | https://docs.datadoghq.com | https://api.ap2.datadoghq.com |
...
- If multiple consecutive fetch attempts fail (e.g., network timeout, DNS resolution failure), inform the user that internet access appears unavailable and suggest: (1) checking network connectivity, (2) visiting the docs URL directly in a browser, or (3) proceeding without live docs using cached knowledge as a last resort — clearly marking any such guidance as potentially outdated.
- If the llms.txt index loads but specific page fetches consistently fail, the documentation site may be experiencing partial outages. Try alternative pages in the same product area and inform the user of the issue.
- If fetched content appears truncated or malformed, retry the fetch once. If the issue persists, use `head` to retrieve a smaller portion and work with available content rather than failing silently.
